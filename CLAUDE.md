...
### 路由总览（100 条 = 98 HTTP 操作 + 2 WebSocket）

- 用户端（`/api/v1`）：`auth/*` `users*` `tasks*` `tags*` `notifications/*` `feedback` `sync/*` — 全部为真实业务
- 管理端（`/api/v1/admin`）：所有 98 个 HTTP 端点均为真实业务实现，无骨架占位。包括：`auth/*` `users*` `dashboard/stats|charts/{metric}` `audit/*` `feedback/*` `config` `sensitive-words/*` `security/ip-blacklist/*` `announcements/*` `login-logs` `tasks*` `tags*` `users/{uuid}/tasks|tags`
...
- `AdminService` 1,439 行（管理员认证、用户管理、仪表盘统计、审计日志、反馈管理、系统配置、内容管理、敏感词、IP黑名单、公告）
...
- `docs/04-工程与部署/工程指南.md` — 部署、测试、CI/CD
